...
if __name__ == "__main__":
    ...
    for episode in range(NUM_EPISODIOS):
        ...
        prev_q_table = {state: actions[:] for state, actions in agent.q_table.items()}
        if episode == 0:
            converged = False
        max_diff = 0
        threshold = 1e-3
        ...
        if not converged:
            ...
            # Comprobamos si la diferencia máxima entre los valores de las acciones de la tabla Q ha sido inferior a un umbral
            for state, actions in agent.q_table.items():
                if state not in prev_q_table: # Si el estado no estaba en la tabla anterior, se añade con valores por defecto
                    prev_q_table[state] = [DEF_Q_V] * len(actions)
                prev_actions = prev_q_table[state]
                # Calculamos la máxima diferencia entre los valores de la acción para este estado
                diff = max(abs(prev_actions[i] - actions[i]) for i in range(len(actions)))
                if diff > max_diff:
                    max_diff = diff
            
            # Si la diferencia máxima es inferior al umbral, se considera que el algoritmo ha convergido
